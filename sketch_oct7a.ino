...
// === Modbus TCP (emelianov/Modbus-ESP8266)
...
#define LED_PIN        2      // Builtin LED (active LOW)

// --- OTA červená LED (fade) ---
#define RED_LED_PIN   15      // GPIO15 (D8). Dejte -1, pokud LED není připojena.
...
// WiFiManager – kolik sekund čekat v STA, než spadne do AP
...
// Telnet stav
static bool telnetEnabled = false;
static bool telnetAuthenticated = false;

// Krátké ID (12 HEX znaků) pro MQTT ClientID, hostname, {id} v topics
...
// >>> Boot červená LED (5 s po startu)
static const uint32_t BOOT_RED_LED_MS = 5000UL;
...
// --- REV COUNTER pro web auto-refresh ---
...
static void fillDefaultConfig(DeviceConfig &cfg) {
  ...
  cfg.version = CONFIG_VER;

  // MQTT
  strlcpy(cfg.mqttServer, "", sizeof(cfg.mqttServer));
  cfg.mqttPort = USE_MQTT_TLS ? 8883 : 1883;
  strlcpy(cfg.mqttUser, "", sizeof(cfg.mqttUser));
  strlcpy(cfg.mqttPassword, "", sizeof(cfg.mqttPassword));
  ...
  // Telnet (bezpečné výchozí nastavení: vypnuto)
  cfg.telnetEnable = 0;
...
static void computeAndStoreCRC(DeviceConfig &cfg) {
  cfg.crc16 = 0;
  cfg.crc16 = crc16_x25(reinterpret_cast<const uint8_t*>(&cfg), sizeof(cfg));
}

static bool isConfigValid(const DeviceConfig &cfg) {
  if (cfg.magic != CONFIG_MAGIC || cfg.version != CONFIG_VER) return false;
  uint16_t saved = cfg.crc16;
  DeviceConfig tmp = cfg;
  tmp.crc16 = 0;
  return saved == crc16_x25(reinterpret_cast<const uint8_t*>(&tmp), sizeof(tmp));
}
...
static void expandTopic(const char* in, char* out, size_t outLen) {
  const char* p = strstr(in, "{id}");
  if (!p) { strlcpy(out, in, outLen); return; }
  size_t head = (size_t)(p - in);
  size_t tail = strlen(p + 4);
  if (head + strlen(deviceID) + tail + 1 > outLen) {
    strlcpy(out, in, outLen);
    return;
  }
  memcpy(out, in, head);
  memcpy(out + head, deviceID, strlen(deviceID));
  memcpy(out + head + strlen(deviceID), p + 4, tail);
  out[head + strlen(deviceID) + tail] = '\0';
}
...
static void processCommand(const char* cmd) {
  ...
  // POWER
  if (!strcasecmp(cmd, "STATE_ON"))  { hvac.setState("on");  lastHvacResponse = millis(); bumpRevIfChanged(); return; }
  if (!strcasecmp(cmd, "STATE_OFF")) { hvac.setState("off"); lastHvacResponse = millis(); bumpRevIfChanged(); return; }
...
if (!strncasecmp(cmd, "MODE_", 5)) {
  ...
  hvac.setMode(m);
  g_reqModeReg = modeToReg(m);     // <<< okamžitě nastav pro Modbus
  lastHvacResponse = millis();
  bumpRevIfChanged();
  return;
}
...
if (!strncasecmp(cmd, "FAN_", 4)) {
  ...
  hvac.setFanMode(f);
  g_reqFanReg = fanToReg(f);        // <<< okamžitě nastav pro Modbus
  lastHvacResponse = millis();
  bumpRevIfChanged();
  return;
}
  ...
  // SWING
  if (!strcasecmp(cmd, "SWING_OFF")) { hvac.setSwing("fix");      lastHvacResponse = millis(); bumpRevIfChanged(); return; }
  if (!strcasecmp(cmd, "SWING_V"))   { hvac.setSwing("v_swing");  lastHvacResponse = millis(); bumpRevIfChanged(); return; }
  if (!strcasecmp(cmd, "SWING_H"))   { hvac.setSwing("h_swing");  lastHvacResponse = millis(); bumpRevIfChanged(); return; }
  if (!strcasecmp(cmd, "SWING_VH"))  { hvac.setSwing("vh_swing"); lastHvacResponse = millis(); bumpRevIfChanged(); return; }
  if (!strncasecmp(cmd, "SWING_FIX_", 10)) {
    ...
    if (p[0]>='1' && p[0]<='5' && p[1]==0) {
      ...
      hvac.setSwing(buf); lastHvacResponse = millis(); bumpRevIfChanged(); return;
    }
...
static uint16_t modeToReg(const char* m) {
  if (!m) return 0;
  if (!strcmp(m,"auto")) return 0;
  if (!strcmp(m,"cool")) return 1;
  if (!strcmp(m,"heat")) return 2;
  if (!strcmp(m,"dry"))  return 3;
  if (!strcmp(m,"fan_only")) return 4;
  return 0;
}

static const char* regToMode(uint16_t r) {
  switch (r) { case 0: return "auto"; case 1: return "cool"; case 2: return "heat"; case 3: return "dry"; case 4: return "fan_only"; default: return "auto"; }
}

// 0=auto, 1..5 = lvl_1..lvl_5  (quiet mapujeme na lvl_1)
static uint16_t fanToReg(const char* f) {
  if (!f) return 0;
  if (!strcmp(f,"auto"))  return 0;
  if (!strcmp(f,"quiet")) return 1;  // kompatibilita
  if (!strcmp(f,"lvl_1")) return 1;
  if (!strcmp(f,"lvl_2")) return 2;
  if (!strcmp(f,"lvl_3")) return 3;
  if (!strcmp(f,"lvl_4")) return 4;
  if (!strcmp(f,"lvl_5")) return 5;
  return 0;
}
...
static uint16_t swingToReg(const char* s) {
  if (!s) return 0;
  if (!strcmp(s,"fix")) return 0;
  if (!strcmp(s,"v_swing")) return 1;
  if (!strcmp(s,"h_swing")) return 2;
  if (!strcmp(s,"vh_swing")) return 3;
  if (!strcmp(s,"fix_pos_1")) return 11;
  if (!strcmp(s,"fix_pos_2")) return 12;
  if (!strcmp(s,"fix_pos_3")) return 13;
  if (!strcmp(s,"fix_pos_4")) return 14;
  if (!strcmp(s,"fix_pos_5")) return 15;
  return 0;
}

static const char* regToSwing(uint16_t r) {
  switch (r) {
    case 0: return "fix";       case 1: return "v_swing";
    case 2: return "h_swing";   case 3: return "vh_swing";
    case 11: return "fix_pos_1"; case 12: return "fix_pos_2";
    case 13: return "fix_pos_3"; case 14: return "fix_pos_4";
    case 15: return "fix_pos_5"; default: return "fix";
  }
}
...
static uint16_t operToReg(const char* oper) {
  ...
  return modeToReg(oper); // auto/cool/heat/dry/fan_only -> 0..4
}
...
static void modbusSyncFromHVAC() {
  ...
  if (g_reqFanReg != fnCur && g_reqFanReg <= 5) {
...
static void modbusApplyWrites() {
  ...
  if (v != hregCache[HREG_MODE]) {
    ...
    g_reqModeReg = v;                   // <<< shadow pro okamžitý export
  ...
  if (v != hregCache[HREG_FAN]) {
    ...
    g_reqFanReg = v;                    // <<< shadow
    hregCache[HREG_FAN] = v;
    lastHvacResponse = millis();
  }

  v = mb.Hreg(HREG_SWING);
  if (!((v<=3) || (v>=11 && v<=15))) v = 0;
...
function bind(){
  ...
  // segments
  const mg = $('#modeGroup');
  if(mg) mg.querySelectorAll('button').forEach(b=>b.onclick=()=>send('MODE_'+b.dataset.val.toUpperCase()));
  const fg = $('#fanGroup');
...
static void handleRoot() {
  ...
  html += F("<div class='header'>"
            "  <div class='hgroup'>"
            "    <span class='brand'>Toshiba HVAC</span>"
            "    <span class='badge'>ID: ");
  html += deviceID;
  ...
  html += F("<div class='card' style='grid-column:span 6'>"
            ...
            */
            "    <button class='btn' data-val='lvl_1'>1</button>"
            "    <button class='btn' data-val='lvl_2'>2</button>"
            "    <button class='btn' data-val='lvl_3'>3</button>"
            "    <button class='btn' data-val='lvl_4'>4</button>"
            "    <button class='btn' data-val='lvl_5'>5</button>"
            "  </div>"
            "</div>");
  ...
  while (telnetClient.available()) {
    ...
    if (ch == '\n') {
      ...
      if (!strcasecmp(line, "help")) {
        ...
        //bool hvacConn = (millis() - lastHvacResponse < HVAC_TIMEOUT_MS);
        const bool hvacConn = (hvac.isConnected() || hvac.isCduRunning());
        float tin, tout;
        bool hasTin  = hvacReadRoomTemp(tin);
        bool hasTout = hvacReadOutdoorTemp(tout);
...
static void bootRedLedHandle() {
  ...
  uint32_t now = millis();
  if (now < bootRedUntil) {
    analogWrite(RED_LED_PIN, 1023); // svítí naplno
  } else {
    bootRedActive = false;
    analogWrite(RED_LED_PIN, 0);    // zhasnout
  }
}
...
static void otaLedHandle() {
  ...
  const uint32_t period = 1200; // ms tam a zpět
  uint32_t t = (millis() - otaStartMs) % period;
  // trojúhelník 0..1023..0
  uint16_t val = (t < period / 2)
                 ? (uint16_t)((t * 1023UL) / (period/2))
                 : (uint16_t)(((period - t) * 1023UL) / (period/2));
